...
La función experimental Flakes es un avance importante para Nix: introduce una política
para gestionar dependencias entre expresiones de Nix y mejora la reproducibilidad, la
...
En esos lenguajes, esos archivos se usan para describir las dependencias entre paquetes de
...
`package-lock.json`/`go.sum`/`Cargo.lock`/`poetry.lock` para fijar las versiones de las
dependencias y asegurar la reproducibilidad de los proyectos.
...
parecido a `package-lock.json`, para fijar las versiones de las dependencias y garantizar
...
que Flakes ofrece una forma nueva y más cómoda de gestionar las dependencias entre
...
:warning: Sin embargo, es importante tener en cuenta que **Flakes sigue siendo una función
experimental**. Persistent algunos problemas y existe la posibilidad de introducir cambios
...
NixOS y Flakes. Sin embargo, conviene estar preparado para posibles problemas derivados de
...
funciones traen una nueva interfaz de línea de comandos (llamada el nuevo CLI), una
definición estandarizada de la estructura de paquetes de Nix (conocida como la función
Flakes) y archivos como `flake.lock`, similares a los archivos de bloqueo de versiones en
...
separarlos de forma explícita, usar Flakes esencialmente requiere usar el nuevo CLI. En
...
reemplazarlos por los comandos correspondientes del nuevo CLI (excepto
`nix-collect-garbage`, porque actualmente no hay alternativa para ese comando):

1. `nix-channel`: `nix-channel` gestiona versiones de entradas como nixpkgs mediante
   ...
   1. En Flakes, la funcionalidad de `nix-channel` la reemplaza el Flake Registry
      ...
      nixpkgs" en el uso interactivo del CLI (por ejemplo, `nix run nixpkgs#hello`). Al
      usar un `flake.nix`, las versiones de las entradas se gestionan dentro del propio
      flake.
   2. Flakes usa la sección `inputs` en `flake.nix` para gestionar las versiones de
      nixpkgs y otras entradas en cada flake, en lugar de usar estado global.
2. `nix-env`: `nix-env` es una herramienta central de línea de comandos del Nix clásico,
   ...
   1. Instala paquetes desde las fuentes de datos añadidas por `nix-channel`, por lo que
      ...
      Por eso, no se recomienda usar este comando directamente.

   2. El comando correspondiente en el nuevo CLI es `nix profile`. Personalmente, no lo
...
5. `nix-collect-garbage`: comando de recolección de basura usado para limpiar objetos del
   almacén no utilizados en `/nix/store`.
   1. Hay un comando similar en el nuevo CLI, `nix store gc --debug`, pero no limpia las
      generaciones de perfil, así que por ahora no hay alternativa para este comando.
6. Y otros comandos menos usados no se enumeran aquí.
   1. Puedes consultar la lista comparativa detallada de comandos en
