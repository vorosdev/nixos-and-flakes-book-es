...
Este patrón de uso ya no se recomienda con la característica Flakes porque have que las
...
usamos con frecuencia el commando `nix repl '<nixpkgs>'`, que utilize el Nixpkgs
encontrado mediante la búsqueda en `NIX_PATH`.
...
en vez de direcciones largas de repositorios cuando usamos commandos como `nix run`,
`nix shell`
...
normalmente queremos que el `nixpkgs` usado en commandos como `nix repl '<nixpkgs>'`,
`nix run nixpkgs#ponysay hello` coincida con el `nixpkgs` del sistema. Esto se have por
...
{ nixpkgs, ... }: {
  ...
  # esto lo establece automáticamente nixpkgs.lib.nixosSystem, pero podría set necesario
